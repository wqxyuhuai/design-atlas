...
import DotField from "../backgrounds/dot-field/DotField";
import GradientBlinds from "../backgrounds/gradient-blinds/GradientBlinds";
import LetterGlitch from "../backgrounds/letter-glitch/LetterGlitch";
import LightRays from "../backgrounds/light-rays/LightRays";
import PixelBlast from "../backgrounds/pixel-blast/PixelBlast";
import Prism from "../backgrounds/prism/Prism";
import PrismaticBurst from "../backgrounds/prismatic-burst/PrismaticBurst";
import ShinyText from "../text/shiny-text/ShinyText";
import Shuffle from "../text/shuffle/Shuffle";
import TextPressure from "../text/text-pressure/TextPressure";
import TextType from "../text/text-type/TextType";
import FlowingMenu from "../media/flowing-menu/FlowingMenu";
import CardSwap, { Card } from "../media/card-swap/CardSwap";
import { StaggeredMenu } from "../navigation/staggered-menu/StaggeredMenu";
import GlassSurface from "../components/glass-surface/GlassSurface";
import FluidGlass from "../components/fluid-glass/FluidGlass";
import SplashCursor from "../interactions/splash-cursor/SplashCursor";
import ImageTrail from "../media/image-trail/ImageTrail";
import ShapeBlur from "../interactions/shape-blur/ShapeBlur";
import Strands from "../components/strands/Strands";
import MagnetLines from "../interactions/magnet-lines/MagnetLines";
import GlareHover from "../interactions/glare-hover/GlareHover";
import RotatingText from "../text/rotating-text/RotatingText";
import VariableProximity from "../text/variable-proximity/VariableProximity";
import TrueFocus from "../text/true-focus/TrueFocus";
import DecryptedText from "../text/decrypted-text/DecryptedText";
...
type SpiralShowreelMotionState = {
  target: number;
  current: number;
  speed: number;
  wheelDirection: 1 | -1;
...
function SpiralShowreelGallery({
  images,
  motionRef,
  compact,
  cardCount,
  ...
  images: PlaceholderImage[];
  motionRef: MutableRefObject<SpiralShowreelMotionState>;
  compact: boolean;
  cardCount: number;
  radius: number;
  bend: number;
  ...
  return (
    <group>
      {textures.slice(0, totalCards).map((texture, index) => (
        <mesh
          key={`${images[index]?.src ?? "spiral"}-${index}`}
          ref={(node) => {
            meshRefs.current[index] = node;
          }}
        >
...
function MediaPreview({ props, renderer, mode }: { props: EffectProps; renderer: string; mode: string }) {
  ...
  const handleSpiralPointer = (event: MouseEvent<HTMLDivElement>) => {
    ...
    spiralMotionRef.current.pointerX = ((event.clientX - rect.left) / rect.width - 0.5) * 2;
    spiralMotionRef.current.pointerY = ((event.clientY - rect.top) / rect.height - 0.5) * 2;
  };

  useEffect(() => {
    if (renderer !== "spiral-showreel-gallery") return undefined;
    const stage = spiralStageRef.current;
    ...
    const handleNativeWheel = (event: globalThis.WheelEvent) => {
      ...
      applySpiralWheelDelta(dominantDelta);
    };
  ...
  }, [applySpiralWheelDelta, renderer]);
  ...
  if (renderer === "spiral-showreel-gallery") {
    ...
    return (
      <div
        ref={spiralStageRef}
        className="notion-rb-media-stage notion-rb-spiral-showreel-stage"
        style={motionStyle}
        onPointerMove={handleSpiralPointer}
      >
        <Canvas
        ...
        >
          <Suspense fallback={null}>
            <SpiralShowreelGallery
              images={spiralImages}
              motionRef={spiralMotionRef}
              compact={compact}
              cardCount={spiralCardCount}
              radius={spiralRadius}
              bend={spiralBend}
              autoSpeed={spiralAutoSpeed}
            />
          </Suspense>
        </Canvas>
      </div>
...
export function NotionInboxPreview(props: EffectProps) {
  ...
  return (
    <div
      className={`imported-effect-preview notion-rb-preview notion-rb-${isText ? "text-effect" : "background-effect"} notion-rb-${renderer} ${
