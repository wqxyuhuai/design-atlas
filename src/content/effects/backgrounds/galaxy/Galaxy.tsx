...
export function Galaxy(props: EffectProps) {
  ...
  const focalX = numberValue(props.focalX, 0.5);
  const focalY = numberValue(props.focalY, 0.5);
  const rotationDeg = numberValue(props.rotation, 0);
  const starSpeed = numberValue(props.starSpeed, 0.5);
  const density = numberValue(props.density, 1);
  const hueShift = numberValue(props.hueShift, 140);
  const speed = numberValue(props.speed, 1);
  const glowIntensity = numberValue(props.glowIntensity, 0.3);
  const saturation = numberValue(props.saturation, 0);
  const mouseRepulsion = booleanValue(props.mouseRepulsion, true);
  const mouseInteraction = booleanValue(props.mouseInteraction, true);
  const repulsionStrength = numberValue(props.repulsionStrength, 2);
  const twinkleIntensity = numberValue(props.twinkleIntensity, 0.3);
  const rotationSpeed = numberValue(props.rotationSpeed, 0.1);
  const autoCenterRepulsion = numberValue(props.autoCenterRepulsion, 0);
  const transparent = booleanValue(props.transparent, true);
  const disableAnimation = booleanValue(props.disableAnimation, false);
